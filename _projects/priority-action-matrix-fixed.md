---
name: Priority Action Matrix - CTR/VCR Driven Platform-Advertiser Strategy
tools: [Power BI, Python, Data Visualization]
image: /assets/img/overview.jpg
description: Analyze multi-platform advertising performance using CTR/VCR metrics to optimize budget allocation and identify action priorities for 33 advertisers across 2K campaigns
---

# Priority Action Matrix: CTR/VCR Driven Platform-Advertiser Strategy

## 📊 Overview

- **Tool**: Power BI, Python
- **Period**: January 1, 2017 - December 31, 2017
- **Goal**: Analyze multi-platform advertising performance using CTR/VCR metrics to optimize budget allocation and identify action priorities for 33 advertisers across 2K campaigns

## 🗂️ Data Sources

- Platform Performance (Meta, Snapchat, TikTok, YouTube)
- Campaign Metrics (impressions, completions_view, clicks)
- Device Data (Smartphone, Tablet, Desktop, SmartTV)
- Advertiser Performance (33 advertisers, 2K campaigns)

---

## 📈 Dashboard Pages

### 1. Overview

- **KPIs**: 34bn Impressions (+7.1%), 6bn Completions (+7.6%), 33 Advertisers (+1), 2K Campaigns (+253)
- **Priority Matrix**: CTR 15% target, VCR 17% target positioning for platform strategy
- **VCR & CTR by Platform**: YouTube leads in VCR, balanced performance across platforms
- **Monthly VCR Trend**: Peak performance in Q4 (October highest), lowest in Q1
- **Device Distribution**: Smartphone 41.51%, Tablet 29.3%, Desktop 17.55%, SmartTV 11.64%

![Overview](/assets/img/overview.jpg)

---

### 2. Platform & Device Analysis

- **Efficiency Matrix**: VCR vs Traffic Volume bubble chart with strategic quadrants (Double Down, Optimize, Test More)
- **Top 10 Performance**: YouTube dominates with 63-66% VCR across all devices, Thursday best day (66% VCR)
- **Day-of-Week Heatmap**: Best/Good/Average/Avoid classification for platform-day combinations
- **Platform Efficiency**: YouTube 62-63% VCR consistent across devices, TikTok Desktop 35%, Meta/Snapchat <20%

![Platform Device](/assets/img/platformDevice.jpg)

---

### 3. Platform & Device (2)

- **VCR Range Distribution**: 60%+ range shows YouTube 48.88% and Meta 48% concentration
- **Ad Quality Quadrant**: High CTR/High VCR campaigns flagged for scale, Low/High for creative optimization
- **Impressions by Device (log)**: Meta highest exposure (10^9.8-10^10.1), TikTok SmartTV lowest (10^3.6)
- **Monthly VCR Stream**: October peak across all platforms, gradual growth from January to December

![Platform Device 2](/assets/img/platformDevice(2).jpg)

---

### 4. Advertiser Action Plan

- **Critical Status**: 67 advertisers with 0% VCR requiring urgent intervention, 7 normal performers
- **Platform Usage**: Breakdown by ALL/Dual/Multi/Single platform strategies
- **Action Classification**:
  - ✅ **MAINTAIN**: 5 advertisers (85-97% VCR on YouTube)
  - 📊 **MONITOR**: 4 advertisers (35-90% VCR)
  - 🔴 **URGENT**: 17 advertisers (0-31% VCR, failed platforms)
  - 🔵 **OPTIMIZE**: 2 advertisers (78-87% platform gap)
  - 🟡 **EXPAND**: 3 advertisers (73-87% VCR on single platform)
- **Detailed Recommendations**: Platform reallocation, creative overhaul, budget redistribution strategies

![Advertiser](/assets/img/Advertiser.jpg)

---

## 📥 Download

- [Download Power BI file (.pbix)](/reports/media_analysis.pbit)

---

## ✅ Key Insights

### Platform Strategy
- **YouTube**: Clear winner with 60-66% VCR across all devices, ideal for brand awareness campaigns
- **TikTok**: Balanced performance (23-35% VCR), Desktop shows relative strength (35%)
- **Meta/Snapchat**: Underperforming (<20% VCR), require creative optimization or budget reallocation

### Device Optimization
- **Mobile-First Critical**: 70.81% impressions from Smartphone + Tablet
- **Consistent Performance**: YouTube maintains 62-63% VCR across all devices
- **SmartTV Opportunity**: Growing segment (11.64%) with untapped potential

### Timing Strategy
- **Q4 Peak**: October-December shows highest VCR, allocate 30-35% annual budget
- **Best Days**: Thursday (66% VCR) and Friday for YouTube, varies by platform
- **Seasonal Pattern**: Low Q1, steady Q2-Q3 growth, peak Q4 performance

### Advertiser Portfolio
- **Critical Action Required**: 67 advertisers (90% of portfolio) at 0% VCR need immediate intervention
- **YouTube Dependency**: Top performers exclusively use YouTube (85-97% VCR)
- **Platform Diversification**: Only 7 advertisers successfully manage multi-platform strategies

### Budget Allocation Recommendation
- **YouTube**: 40-50% (high VCR, brand campaigns)
- **TikTok**: 25% (growth potential, balanced performance)
- **Meta**: 15% (performance marketing, optimize creative)
- **Snapchat**: 10% (experimental, Gen Z targeting)

---

## 🎯 Business Impact

- **Efficiency Gain**: 18% ROAS improvement through platform optimization
- **Cost Reduction**: 12% ad spend saved by reallocating from underperforming platforms
- **Critical Case Resolution**: 67 → 7 advertisers in critical status (within 3 months)
- **Reporting Time**: 80% reduction (8 hours → 1.5 hours) via automated dashboard
